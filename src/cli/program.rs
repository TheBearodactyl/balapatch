use crate::adb::{ConnectionMode, get_adb_connection};
use crate::cli::clap_cli::{AdbSubcommands, BalatroSubcommands, Cli, Commands, ConnectSubcommands};
use crate::progress::{GLOBAL_MP, create_spinner};
...
pub async fn program() -> anyhow::Result<()> {
    ...
    match argv.command {
        Commands::Adb { command } => match command {
            AdbSubcommands::Connect { command } => match command {
                ConnectSubcommands::Wireless {
                    ip_addr,
                    port,
                    adb_pin,
                } => {
                    let spinner = create_spinner("Establishing wireless connection...");
                    let connection = get_adb_connection(ConnectionMode::Wireless((ip_addr, port)))?;
                ...
                ConnectSubcommands::Wired => {
                    let spinner = create_spinner("Checking USB devices...");
                    let adb_connect = Command::new("adb").arg("devices").output();
                    let mut output_str: String = "".to_string();

                    if adb_connect.is_ok() {
                        for ch in adb_connect?.stdout {
                            output_str.push(ch as char);
                        }

                        spinner.finish_with_message("USB connection established");
                        info!("Successfully connected to USB ADB device!");
                        info!(
                            "Device ID =====> {}",
                            output_str.split("\n").nth(1).unwrap().replace("device", "")
                        );
                    } else {
                        spinner.finish_with_message("Failed to connect to USB device");
                        error!("Failed to connect to USB ADB device");
                    }
                }
            },
